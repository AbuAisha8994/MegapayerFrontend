...
const Wallet = ({ position, color, label, onClick, isActive }: { 
  ...
  useFrame(({ clock }) => {
    if (meshRef.current) {
      // Floating animation
      meshRef.current.position.y = position[1] + Math.sin(clock.getElapsedTime() * 0.8 + position[0]) * 0.1;
...
const Escrow = ({ position, isActive }: { position: [number, number, number], isActive: boolean }) => {
  const meshRef = useRef<THREE.Mesh>(null);
...
// Transaction asset
const Asset = ({ position, targetPosition, progress, color }: { 
  position: [number, number, number], 
...
const P2PAnimation = () => {
  ...
  const buyer = { position: [-2, 0, 0] as [number, number, number], color: "#4f46e5", label: "BUYER" };
  const seller = { position: [2, 0, 0] as [number, number, number], color: "#f59e0b", label: "SELLER" };
  ...
  useFrame(({ clock }) => {
    ...
    if (transactionsRef.current) {
      ...
      transactionsRef.current.children.forEach((txObj, i) => {
        ...
        const t1 = 1 - p;
        const t2 = p;
        txObj.position.x = (t1 * t1) * fromPos[0] + 2 * t1 * t2 * midPoint[0] + (t2 * t2) * toPos[0];
        txObj.position.y = (t1 * t1) * fromPos[1] + 2 * t1 * t2 * midPoint[1] + (t2 * t2) * toPos[1];
        txObj.position.z = (t1 * t1) * fromPos[2] + 2 * t1 * t2 * midPoint[2] + (t2 * t2) * toPos[2];
      });
    }
  });
  ...
  return (
    <group ref={groupRef}>
      ...
      <group ref={transactionsRef}>
        {transactions.map((tx, index) => (
          <Box 
            ...
            position={[tx.from.position[0], tx.from.position[1], tx.from.position[2]]}
          >
            <meshStandardMaterial 
              color={tx.color}
              metalness={0.8}
        ...
        {nodes.map((node, i) => {
          ...
          return connections.map((conn, connIndex) => (
            <line key={`line-${i}-${connIndex}`}>
              ...
              <lineBasicMaterial color={conn.color} transparent opacity={conn.opacity} />
            </line>
          ));
        })}
      </group>
      ...
      <Wallet 
        ...
        isActive={isTransactionActive && (transactionStep === 0 || transactionStep === 3)}
      />
