...
const NetworkNode = ({ position, size, color, pulseSpeed = 1 }: { 
  ...
  useFrame(({ clock }) => {
    if (meshRef.current) {
      // Floating motion
      meshRef.current.position.y = scaledPosition[1] + Math.sin(clock.getElapsedTime() * 2 * pulseSpeed + scaledPosition[0]) * 0.05;
    ...
    if (glowRef.current && glowRef.current.material instanceof THREE.Material) {
      ...
      material.opacity = (Math.sin(clock.getElapsedTime() * 2 * pulseSpeed) * 0.4 + 0.6);
    }
  });
...
const ConnectionLine = ({ startPos, endPos, color }: { 
  startPos: [number, number, number], 
  endPos: [number, number, number], 
  color: string 
}) => {
  // Scale down positions to make network more compact
  const scaledStartPos: [number, number, number] = [startPos[0] * 0.7, startPos[1] * 0.7, startPos[2] * 0.7];
  const scaledEndPos: [number, number, number] = [endPos[0] * 0.7, endPos[1] * 0.7, endPos[2] * 0.7];
  ...
  useFrame(({ clock }) => {
    if (ref.current) {
      ...
      ref.current.material.opacity = (Math.sin(clock.getElapsedTime() * 2) * 0.3 + 0.7);
    }
    ...
    if (energyRef.current) {
      const x = scaledStartPos[0] + (scaledEndPos[0] - scaledStartPos[0]) * normalizedPos;
      const y = scaledStartPos[1] + (scaledEndPos[1] - scaledStartPos[1]) * normalizedPos;
      const z = scaledStartPos[2] + (scaledEndPos[2] - scaledStartPos[2]) * normalizedPos;
    ...
    <div className="relative h-screen w-full overflow-hidden">
      ...
      <div className="absolute inset-0 flex items-center justify-center z-20">
        ...
        >
          ...
          >
            <span className="block text-gradient-gold mb-2 text-xl md:text-2xl shadow-lg">2025 - Next Generation DeFi</span>
            <span className="text-gradient text-7xl md:text-8xl font-extrabold [text-shadow:_0_5px_20px_rgb(79_70_229_/_40%)]">MEGA<span className="text-white [text-shadow:_0_5px_15px_rgb(255_255_255_/_30%)]">PAYER</span></span>
          </motion.div>
          ...
          >
            The next generation blockchain ecosystem powering the future of decentralized finance
          </motion.p>
      ...
      <div className="absolute bottom-10 left-0 right-0 text-center z-20">
        ...
        >
          <svg className="w-8 h-8 mx-auto text-white/70 drop-shadow" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
